...
def plot_tracking_results(csv_path=None, output_png=None, case_title="Case 1 (5-Param Min-Jerk Flatness)"):
    ...
    # ENU velocities: if recorded with legacy node (without 'vx'), negate v and r
    if 'vx' in data:
        ...
        u_real = clean_outliers(data['u_real'])
        v_real = clean_outliers(-data['v_real'])  # Legacy CSV had NED vy
        r_real = clean_outliers(-data['r_real'])  # Legacy CSV had NED wz
...
if __name__ == '__main__':
    ...
    plot_tracking_results(csv_arg, out_arg, case_title="Case 1 (5-Param Min-Jerk Flatness)")
