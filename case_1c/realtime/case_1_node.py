#!/usr/bin/env python3
"""
Real-Time ROS 2 Node for Case 1: Minimum Jerk QP + 5-Parameter Exact Flatness (m11 = m22).
...
3. Executes planning + flatness reconstruction (one-shot).
...
# Path resolution to load Case 1 modules
def _resolve_case1_dir():
    candidates = [
        Path(__file__).resolve().parent.parent.parent / 'case_1',
        Path('/home/brayan/ros2_ws/src/min-jerk-flatness-planning/case_1'),
    ]
    try:
        from ament_index_python.packages import get_package_share_directory
        share_dir = Path(get_package_share_directory('min-jerk-flatness-planning'))
        candidates.insert(0, share_dir / 'case_1')
    except Exception:
        pass
    for cand in candidates:
        if cand is not None and cand.is_dir():
            return cand
    raise RuntimeError("Could not find case_1 directory containing min_jerk_qp module.")

CASE1_PY_DIR = _resolve_case1_dir()
if str(CASE1_PY_DIR) not in sys.path:
    sys.path.insert(0, str(CASE1_PY_DIR))

# Default output directory in source workspace realtime folder
DEFAULT_OUTPUT_DIR = Path('/home/brayan/ros2_ws/src/min-jerk-flatness-planning/case_1c/realtime')
if not DEFAULT_OUTPUT_DIR.exists():
    DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent

from min_jerk_qp import MinJerkTrajectory2D
from flatness_reconstruct import reconstruct_flatness_h2
from usv_params import DT_SIM, thrust_from_cmd_richards


class Case1RealTimeNode(Node):
    def __init__(self):
        super().__init__(
            'case_1_realtime_node',
        ...
        self.get_logger().info('Case 1 Real-Time Node Initialized. Waiting for first odometry message...')
    ...
    def plan_trajectory(self):
        ...
        self.get_logger().info('Launching Case 1 C Planner (Min-Jerk QP + 5-Param Exact Flatness in C)...')
        ...
        candidate_bins = [
            ...
            Path('/home/brayan/ros2_ws/src/min-jerk-flatness-planning/case_1c/realtime/main'),
            Path(__file__).resolve().parent / 'case_1_planner',
        ]
        bin_path = None
        ...
        if bin_path is None:
            c_dir = Path('/home/brayan/ros2_ws/src/min-jerk-flatness-planning/case_1c/realtime')
            self.get_logger().info(f'Compiling Case 1 C planner in {c_dir}...')
            import subprocess
        ...
        self.get_logger().info(f'Case 1 Reference loaded successfully: {self.total_steps} steps (duration: {self.t_plan[-1]:.1f} s).')
    ...
    def timer_callback(self):
        ...
        tau_u_act = 2.0 * (T1_act + T2_act)
        tau_r_act = 0.29 * 2.0 * (T1_act - T2_act)
