/* Technique: Case 1 - Minimum Jerk QP Trajectory Planning +
 *            5-Parameter Exact Flatness Reconstruction (m11 = m22)
 *
 * Real-time planner CLI executable:
 * Usage: ./main [x0 y0 psi0 u0 v0 r0 output_csv_path planning_metrics_json]
 *
 * Receives the current vehicle state, transforms waypoints, computes
 * the 5-param Min-Jerk QP + exact flatness in C, and exports the full
 * planned trajectory to CSV for the ROS 2 node.
 */
...
int main(int argc, char *argv[]) {
  /* 1. Parse vehicle state arguments */
  ...
  /* 3. Transform waypoints to start at (x0, y0) with orientation psi0 */
  ...
  /* 4. Velocity boundary conditions in inertial frame */
  ...
  /* 6. Exact Flatness Reconstruction (timed) */
  ...
  /* 7. Export planned reference CSV for ROS 2 node and MPC */
