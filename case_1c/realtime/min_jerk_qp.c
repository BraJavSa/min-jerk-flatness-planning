...
/* Eliminación gaussiana con pivoteo parcial. Resuelve A x = b in-place. */
...
double minjerk1d_eval(const MinJerk1D *tr, double t, int order) {
    /* Replica np.searchsorted(times, t, side='right') - 1:
     * k = mayor indice i tal que times[i] <= t  (i.e. el segmento [times[k], times[k+1]) que contiene a t) */
    int n_seg = tr->n_seg;
    int k = 0;
