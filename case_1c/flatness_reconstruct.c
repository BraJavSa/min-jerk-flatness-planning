...
/* Núcleo por-muestra: dado psi, r, dr ya calculados, hace la inversión de
 * planitud -> fuerzas -> asignación de empuje -> curva de motores.
 * Esta es la función que un controlador en tiempo real llamaría en cada
 * ciclo (no necesita historial, solo el instante actual). */
...
void reconstruct_flatness_h2(const double *x, const double *y,
    ...
    /* np.gradient (2do orden, uniforme en dt) para psi -> r -> dr */
