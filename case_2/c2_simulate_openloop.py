...
# validated in open loop against the full 9-Parameter identified dynamics plant.
...
from c2_usv_params import (
    m11_9, m22_9, m33_9, Xu_9, Xuu_9, Yv_9, Yvv_9, Nr_9, Nrr_9,
    dP_9, DT_SIM, T_MAX, T_MIN
)
...
def real_9param_rk4_step(state, Tu, Tr, dt):
    ...
    def deriv(s):
        ...
        du = (Tu + m22_9 * v_i * r_i - Xu_9 * u_i - Xuu_9 * u_i * abs(u_i)) / m11_9
        dv = (-m11_9 * u_i * r_i - Yv_9 * v_i - Yvv_9 * v_i * abs(v_i)) / m22_9
        dr = (Tr + (m11_9 - m22_9) * u_i * v_i - Nr_9 * r_i - Nrr_9 * r_i * abs(r_i)) / m33_9
...
def main():
    ...
    print("\n==================================================")
    print("CASE 2 SEMILLA OPEN-LOOP TRAJECTORY METRICS (9-PARAM):")
    ...
    axs[0].plot(eta_ref[:, 0], eta_ref[:, 1], color=c_plan, lw=2.2, ls='--', label='Planned (9-Param)')
    ...
    axs[1].plot(t_sim, nu_ref[:, 0], color=c_plan, ls='--', lw=1.8, label='Planned (9-Param)')
    ...
    axs[2].plot(t_sim, nu_ref[:, 1], color=c_green_plan, ls='--', lw=1.8, label='Planned (9-Param)')
    ...
    axs[3].plot(t_sim, nu_ref[:, 2], color=c_amber_plan, ls='--', lw=1.8, label='Planned (9-Param)')
