#!/usr/bin/env python3
# Technique: Case 1 - Minimum Jerk QP Trajectory Planning (5-Param Flatness)
# validated in open loop against the 9-Parameter (full nonlinear) identified
# dynamics -- i.e. the commands are computed via the 5-param flatness
# inversion, but applied to the full 9-param plant to check tracking error.
...
def real_full_dynamics_rk4_step(state, Tu, Tr, dt):
    """RK4 step of the true (9-parameter, full nonlinear) plant."""
    def deriv(s):
        _, _, psi_i, u_i, v_i, r_i = s
        dx = u_i * np.cos(psi_i) - v_i * np.sin(psi_i)
        dy = u_i * np.sin(psi_i) + v_i * np.cos(psi_i)
        dpsi = r_i
        du = (Tu + m22_full * v_i * r_i - Xu_full * u_i - Xuu_full * abs(u_i) * u_i) / m11_full
        dv = (-m11_full * u_i * r_i - Yv_full * v_i - Yvv_full * abs(v_i) * v_i) / m22_full
        dr = (Tr - (m22_full - m11_full) * u_i * v_i - Nr_full * r_i - Nrr_full * abs(r_i) * r_i) / m33_full
...
def main():
    ...
    for i in range(n_steps - 1):
        ...
        state_real = real_full_dynamics_rk4_step(state_real, Tu_apply, Tr_apply, DT_SIM)
    ...
    metrics = {
        "case": "Case 1",
        "solver_type": "QP (5-Param Flatness) validated vs 9-Param (Full Dynamics) plant",
    ...
    print("\n==================================================")
    print("CASE 1 OPEN-LOOP TRAJECTORY METRICS (vs 9-Param full dynamics):")
    ...
    axs[0].plot(eta_ref[:, 0], eta_ref[:, 1], color=c_plan, lw=2.2, ls='--', label='Planned (5-Param)')
    ...
    axs[1].plot(t_sim, nu_ref[:, 0], color=c_plan, ls='--', lw=1.8, label='Planned (5-Param)')
    ...
    axs[2].plot(t_sim, nu_ref[:, 1], color=c_green_plan, ls='--', lw=1.8, label='Planned (5-Param)')
    ...
    axs[3].plot(t_sim, nu_ref[:, 2], color=c_amber_plan, ls='--', lw=1.8, label='Planned (5-Param)')
    ...
    plt.tight_layout()
    out_img_path = os.path.join(script_dir, 'openloop_simulation_results.png')
