...
from usv_params import (
    m_5, m33_5, Xu_5, Yv_5, Nr_5,
    SURGE_GAIN, YAW_ARM, T_MAX, T_MIN,
    cmd_from_thrust_array, thrust_from_cmd_poly,
)
...
def reconstruct_flatness_h2(pos, vel, acc, jerk, t):
    ...
    # Thrust allocation consistent with the identification convention:
    #   tau_u = SURGE_GAIN * (T1 + T2)
    #   tau_r = 2 * YAW_ARM * (T1 - T2)
    T1_raw = 0.5 * (tau_u_raw / SURGE_GAIN + tau_r_raw / (2.0 * YAW_ARM))
    T2_raw = 0.5 * (tau_u_raw / SURGE_GAIN - tau_r_raw / (2.0 * YAW_ARM))

    tau_u_max = SURGE_GAIN * 2.0 * T_MAX
    tau_u_min = SURGE_GAIN * 2.0 * T_MIN
    tau_r_max = 2.0 * YAW_ARM * (T_MAX - T_MIN)
    tau_r_min = -tau_r_max
    ...
    tau_u_act = SURGE_GAIN * (T1_act + T2_act)
    tau_r_act = 2.0 * YAW_ARM * (T1_act - T2_act)
