...
def main():
    ...
    csv_path = os.path.join(script_dir, 'case3_openloop_results.csv')
    with open(csv_path, 'w') as fc:
        fc.write("t,x_ref,y_ref,psi_ref,u_ref,v_ref,r_ref,tau_u_ref,tau_r_ref,T1_ref,T2_ref,"
        ...
        for i in range(n_steps):
            fc.write(f"{t_sim[i]:.6f},{eta_ref[i,0]:.6f},{eta_ref[i,1]:.6f},{eta_ref[i,2]:.6f},"
                     ...
                     f"{tau_ref[i,0]:.6f},{tau_ref[i,1]:.6f},{T_plan[i,0]:.6f},{T_plan[i,1]:.6f},"
                     f"{hist_state_real[i,0]:.6f},{hist_state_real[i,1]:.6f},{hist_state_real[i,2]:.6f},"
                     f"{hist_state_real[i,3]:.6f},{hist_state_real[i,4]:.6f},{hist_state_real[i,5]:.6f}\n")
    ...
    pdf_img_path = os.path.join(script_dir, 'openloop_simulation_results.pdf')
    plt.savefig(pdf_img_path, format='pdf', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Open-loop simulation completed. Plots saved to:\n  PNG: {out_img_path}\n  PDF: {pdf_img_path}")
