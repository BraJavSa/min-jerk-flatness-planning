...
def main():
    ...
    csv_path = os.path.join(script_dir, 'case3_planning_results.csv')
    with open(csv_path, 'w') as fc:
        fc.write("t,x,y,psi,u,v,r,tau_u,tau_r,tau_v,T1,T2,jerk_x,jerk_y\n")
        for i in range(len(t_sim)):
            fc.write(f"{t_sim[i]:.6f},{eta_ref[i,0]:.6f},{eta_ref[i,1]:.6f},{eta_ref[i,2]:.6f},"
                     f"{nu_ref[i,0]:.6f},{nu_ref[i,1]:.6f},{nu_ref[i,2]:.6f},"
                     f"{tau_ref[i,0]:.6f},{tau_ref[i,1]:.6f},{tau_v[i]:.6f},"
                     f"{T_plan[i,0]:.6f},{T_plan[i,1]:.6f},"
                     f"{jerk[i,0]:.6f},{jerk[i,1]:.6f}\n")
    ...
    axs[2].plot(t_sim, tau_v, color=c_purple, lw=1.8, label='$\\tau_v$ [N]')
    axs[2].axhline(EPSILON_TV, color=c_red, ls='--', lw=1.2, label=f'$+\\epsilon$ ({EPSILON_TV:.1f} N)')
    axs[2].axhline(-EPSILON_TV, color=c_red, ls='--', lw=1.2, label=f'$-\\epsilon$ ({-EPSILON_TV:.1f} N)')
    axs[2].set_xlabel('Time [s]', fontweight='bold')
    axs[2].set_ylabel('Force [N]', fontweight='bold')
    axs[2].set_title('Fictitious Sway Force', fontsize=12, fontweight='bold', pad=8)
    ...
    plt.tight_layout()
    out_img_path = os.path.join(script_dir, 'flatness_nlp_results.png')
    plt.savefig(out_img_path, dpi=300, bbox_inches='tight')
    pdf_img_path = os.path.join(script_dir, 'flatness_nlp_results.pdf')
    plt.savefig(pdf_img_path, format='pdf', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"[Main] Planning completed. Diagnostic plots saved to:\n  PNG: {out_img_path}\n  PDF: {pdf_img_path}")
