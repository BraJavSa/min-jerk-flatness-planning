...
class Case3MpcNode : public rclcpp::Node {
public:
    Case3MpcNode() : Node("case3_mpc_node") {
        ...
        this->declare_parameter<std::string>("output_dir",
            "/home/brayan/ros2_ws/src/min-jerk-flatness-planning/Fictitious-Input_Full_Actuation/RealtimeController/output");
        ...
        pub_telemetry_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
            "/case3_mpc/telemetry", 10);

        sub_odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
            odom_topic, 10, std::bind(&Case3MpcNode::odomCallback, this, _1));

        rclcpp::QoS latched_qos(1);
        latched_qos.transient_local();
        sub_ref_path_ = this->create_subscription<std_msgs::msg::String>(
            "/case3/reference_csv_path", latched_qos,
            std::bind(&Case3MpcNode::refPathCallback, this, _1));

        nmpc_ = std::make_unique<NmpcFlatness>(dt_);

        timer_ = this->create_wall_timer(
            std::chrono::duration<double>(dt_),
            std::bind(&Case3MpcNode::controlLoop, this));

        RCLCPP_INFO(this->get_logger(),
                    "Case 3 MPC Node initialized (Horizon N=%d, dt=%.4f s, 6-Param Fictitious-Input NLP MPC). "
                    "Waiting for reference trajectory and odometry...",
                    NmpcFlatness::N, dt_);
    }

    ~Case3MpcNode() override {
...
private:
    ...
    double raw_vx_ = 0.0, raw_vy_ = 0.0, raw_wz_ = 0.0;
    ...
    void refPathCallback(const std_msgs::msg::String::SharedPtr msg) {
        ...
        std::string path_str = msg->data;
        if (loadReferenceCsv(path_str)) {
            reference_ready_ = true;
            RCLCPP_INFO(this->get_logger(),
                        "Reference trajectory loaded: %zu points from %s",
                        ref_.size(), path_str.c_str());
        } else {
            RCLCPP_ERROR(this->get_logger(),
                         "Failed to load reference CSV: %s", path_str.c_str());
        }
    }

    bool loadReferenceCsv(const std::string &filepath) {
    ...
    void controlLoop() {
        ...
        if (!started_) {
            ...
            RCLCPP_INFO(this->get_logger(), "Starting 30Hz closed-loop Case 3 NMPC control execution...");
        }
    ...
    void saveResults() {
        ...
        if (jf.is_open()) {
            jf << "{\n"
               << "    \"case\": \"Case 3 (Fictitious-Input 6-Param Pure Flatness Receding-Horizon NLP MPC, N=30)\",\n"
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<Case3MpcNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
