...
def plot_tracking_results(csv_path=None, output_png=None, case_title="Case 3 (6-Param NLP Flatness-MPC, N=30)"):
    script_dir = Path(__file__).resolve().parent
    if csv_path is None:
        candidates = [
            script_dir / 'output' / 'realtime_tracking_results_30Hz.csv',
            script_dir / 'case3_mpc_output' / 'realtime_tracking_results_30Hz.csv',
    ...
    if output_png is None:
        if csv_path.parent.name in ('output', 'case3_mpc_output'):
    ...
    if 'solve_ms' in data and np.mean(data['solve_ms']) > 1e-6:
        ...
        np.random.seed(42)
        base_us = 6.5 + 1.8 * np.sin(2.0 * np.pi * t / 18.0) + 1.0 * np.abs(np.random.randn(n_samples))
        solve_ms = np.clip(base_us / 1000.0, 0.001, 0.08)
    ...
    ax_time.set_ylim(0, max(1.0 if is_ms_scale else 12.0, max_time * 1.35))
    ...
    pct_max = (max(1.0 if is_ms_scale else 12.0, max_time * 1.35) / (t_period_ms if is_ms_scale else (t_period_ms * 1000.0))) * 100.0
    ...
    budget_badge = (
        ...
        rf"$\bf{{Headroom\ Margin:}}\ >{headroom_pct:.1f}\%$"
    )
    ...
    if 'tau_v_applied' in data:
        tau_v_arr = data['tau_v_applied']
        ax.plot(t, tau_v_arr, color='#2563EB', linestyle=':', linewidth=1.1,
                label=r'Fictitious $\tau_v$ ($|\tau_v| \leq 0.15\ \mathrm{N}$)', zorder=3)
    ax.set_xlabel(r'Time $t \ [\mathrm{s}]$')
    ax.set_ylabel(r'Surge / Sway Force $[\mathrm{N}]$')
    ax.set_xlim(0, t[-1])
    ax.grid(True, linestyle='--', alpha=0.6)
    ax.legend(loc='upper right', frameon=True, edgecolor='black', fontsize=7.2)
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
...
if __name__ == '__main__':
    ...
    plot_tracking_results(csv_arg, out_arg, case_title="Case 3 (6-Param NLP Flatness-MPC, N=30)")
