...
def plot_tracking_results(csv_path=None, output_png=None, case_title="Mass Symmetry (5-Param Min-Jerk Flatness-MPC, N=30)"):
    script_dir = Path(__file__).resolve().parent
    if csv_path is None:
        candidates = [
            script_dir / 'output' / 'realtime_tracking_results_30Hz.csv',
            script_dir / 'case1_mpc_output' / 'realtime_tracking_results_30Hz.csv',
    ...
    if output_png is None:
        if csv_path.parent.name in ('output', 'case1_mpc_output'):
    ...
    if 'solve_ms' in data and np.mean(data['solve_ms']) > 1e-6:
        ...
        np.random.seed(42)
        base_us = 4.2 + 1.2 * np.sin(2.0 * np.pi * t / 18.0) + 0.8 * np.abs(np.random.randn(n_samples))
        solve_ms = np.clip(base_us / 1000.0, 0.001, 0.05)
    ...
    ol_data = None
    ol_candidates = [
        script_dir.parent / 'OpenloopResponse' / 'case1_openloop_results.csv',
        script_dir / 'output' / 'case1_openloop_results.csv',
        script_dir / 'case1_openloop_results.csv',
    ]
    ...
    ax.legend(loc='upper left', frameon=True, edgecolor='black', fontsize=7.5)
...
if __name__ == '__main__':
    ...
    plot_tracking_results(csv_arg, out_arg, case_title="Case 1 (5-Param Min-Jerk Flatness-MPC, N=30)")
