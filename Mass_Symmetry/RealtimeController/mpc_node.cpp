...
class Case1MpcNode : public rclcpp::Node {
public:
    Case1MpcNode() : Node("case1_mpc_node") {
        ...
        this->declare_parameter<std::string>("output_dir",
            "/home/brayan/ros2_ws/src/min-jerk-flatness-planning/Mass_Symmetry/RealtimeController/output");
        ...
        pub_telemetry_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
            "/case1_mpc/telemetry", 10);

        sub_odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
            odom_topic, 10, std::bind(&Case1MpcNode::odomCallback, this, _1));

        rclcpp::QoS latched_qos(1);
        latched_qos.transient_local();
        sub_ref_path_ = this->create_subscription<std_msgs::msg::String>(
            "/case1/reference_csv_path", latched_qos,
            std::bind(&Case1MpcNode::refPathCallback, this, _1));
...
private:
    ...
    void controlLoop() {
        ...
        if (!started_) {
            ...
            RCLCPP_INFO(this->get_logger(), "Starting 30Hz closed-loop NMPC control execution...");
        }
        ...
        double T1_act = thrust_from_cmd(cmd_l);
        double T2_act = thrust_from_cmd(cmd_r);
    ...
    void saveResults() {
        ...
        f.close();
        RCLCPP_INFO(this->get_logger(), "Tracking results (%zu samples) saved to: %s",
                    log_t_.size(), csv_path.c_str());

        double rmse = std::sqrt(sum_sq / log_t_.size());
        ...
        if (jf.is_open()) {
            jf << "{\n"
               ...
               << "}\n";
            jf.close();
            RCLCPP_INFO(this->get_logger(), "Real-time metrics saved to: %s", json_path.c_str());
        }
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<Case1MpcNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
