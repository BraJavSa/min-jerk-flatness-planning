...
int main(void)
{
    ...
    printf("==================================================\n");
    printf("CASE 2 (C) - QP Planning + Pseudo-Flatness Reconstruction\n");
    printf("--------------------------------------------------\n");
    printf("Samples:                        %d (dt = %.5f s, T = %.3f s)\n",
           N, DT_SIM, t_sim[N - 1] - t_sim[0]);
    printf("QP Planning Time:               %.5f ms\n", qp_time_ms);
    printf("Pseudo-Flatness Reconstruction: %.5f ms\n", flatness_time_ms);
    ...
    if (fm) {
        fprintf(fm,
            "{\n"
            "    \"case\": \"Case 2 Pseudo-Flat (C)\",\n"
            "    \"solver_type\": \"QP (6-Param Pseudo-Flatness)\",\n"
            "    \"qp_planning_time_ms\": %.5f,\n"
            "    \"pseudo_flatness_reconstruction_time_ms\": %.5f,\n"
            "    \"total_time_ms\": %.5f,\n"
            "    \"tiempo_qp_ms\": %.5f,\n"
