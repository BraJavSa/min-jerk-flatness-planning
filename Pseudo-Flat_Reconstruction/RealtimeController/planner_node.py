...
def find_planner_binary(configured_path: str = '') -> Path:
    ...
    try:
        ...
        candidates.append(prefix / 'lib' / 'min-jerk-flatness-planning' / 'pseudo_flat_c_planner')
        candidates.append(prefix / 'lib' / 'min-jerk-flatness-planning' / 'case2_mpc_c_planner')
        candidates.append(prefix / 'lib' / 'min-jerk-flatness-planning' / 'case_2_planner')
    except Exception:
        pass

    ws_dir = Path(__file__).resolve().parent
    candidates.append(ws_dir / 'main')
    candidates.append(ws_dir / 'pseudo_flat_c_planner')
    candidates.append(ws_dir / 'case2_mpc_c_planner')

    for name in ('pseudo_flat_c_planner', 'case2_mpc_c_planner'):
...
class Case2PlannerNode(Node):
    def __init__(self):
        ...
        self.pub_ref_path = self.create_publisher(String, '/case2/reference_csv_path', latched_qos)

        self.timer = self.create_timer(0.1, self.tick)
        self.get_logger().info('Case 2 Planner Node initialized. Waiting for first odometry...')
    ...
    def plan_trajectory_once(self):
        ...
        self.get_logger().info(f'Executing Case 2 C planner: {self.planner_binary}')
        ...
        self.get_logger().info('Planned trajectory published successfully to /case2/reference_csv_path')

def main(args=None):
    rclpy.init(args=args)
    node = Case2PlannerNode()
    try:
        rclpy.spin(node)
