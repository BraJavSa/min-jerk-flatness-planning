 ...
 *
 * The original Python implementation builds a KKT system
 *   [ H  A^T ] [c]   [0]
 *   [ A   0  ] [l] = [b]
 * and solves it with a least-squares solve (np.linalg.lstsq). Here we
 * solve the (square) KKT system directly with Gaussian elimination with
 * partial pivoting, which is equivalent for the well-posed systems that
 * arise from this trajectory formulation.
 */
...
/* ----------------------------------------------------------------------
 * Small dense linear solver: solves A x = b for an n x n system using
 * Gaussian elimination with partial pivoting. A is given row-major and
 * is destroyed; b is destroyed and overwritten with the solution x.
 * -------------------------------------------------------------------- */
static void solve_linear_system(double *A, double *b, int n)
{
    ...
    for (k = 0; k < n; ++k) {
        ...
        if (fabs(A[k * n + k]) < 1e-14) {
            A[k * n + k] += 1e-12; /* regularize a near-singular pivot */
        }
...
/* Polynomial basis for a quintic (order-5) polynomial, evaluated at tau,
 * for the given derivative order. basis has length N_COEF. */
...
/* Segment cost (Hessian) matrix for minimum-jerk cost over duration T. */
                 ...
                 int n_wp, double v0, double vf)
{
    ...
    /* Build dense Hessian H (n_vars x n_vars), block-diagonal. */
    ...
    /* Build constraint rows A (m x n_vars) and rhs b (m). */
    /* Count constraints:
     *   2 per segment (position at both ends)      -> 2 * n_seg
     *   1 initial velocity, 1 final velocity        -> 2
     *   1 initial accel = 0, 1 final accel = 0       -> 2
     *   2 per internal junction (vel + accel cont.)  -> 2 * (n_seg - 1)
     */
...
double mjt1d_eval(const MinJerkTrajectory1D *tr, double t, int order)
{
    ...
    /* Equivalent to np.searchsorted(times, t, side='right') - 1:
     * count how many breakpoints are <= t, then subtract 1. */
    {
        int count_le = 0;
        for (i = 0; i < tr->n_wp; ++i) {
